...
"""
    ...
    def create_vacancy_from_hh(cls, vacancy_info_hh: dict):
        """
        Класс метод преобразующий информацию о вакансии из hh.ru
        и создающий объект Vacancy.
        """
        result = {
            ...
            "data_published": cls.check_for_availability(
                vacancy_info_hh, "published_at"
            ),
            "salary_from": cls.check_for_availability(
                vacancy_info_hh, "salary", "from"
            ),
            "salary_to": cls.check_for_availability(vacancy_info_hh, "salary", "to"),
            "currency": cls.check_for_availability(
                vacancy_info_hh, "salary", "currency"
            ),
            ...
            "employer_id": cls.check_for_availability(
                vacancy_info_hh, "employer", "id"
            ),
            "employer_url": cls.check_for_availability(
                vacancy_info_hh, "employer", "alternate_url"
            ),
            "requirement": cls.check_for_availability(
                vacancy_info_hh, "snippet", "requirement"
            ),
            "experience": cls.check_for_availability(
                vacancy_info_hh, "experience", "name"
            ),
            "employment": cls.check_for_availability(
                vacancy_info_hh, "employment", "name"
            ),
        }
    ...
    def check_for_availability(
        vacancy_information: dict, param1: str, param2: str = None, param3: str = None
    ):
        """
        Статический метод используется при создании вакансии объекта.
        Проверка параметра на доступность.
        """
        try:
            if param3 is None:
                if param2 is None:
